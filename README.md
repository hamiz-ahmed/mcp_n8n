...
Just follow the installation steps above and run locally.

### Production Deployment

#### Option 1: Deploy to a Server

1. Clone the repository on your server
2. Install dependencies: `npm install --production`
3. Set environment variables (use your server's environment variable system)
...
docker run -e OPENAI_API_KEY=your-key mcp-client
```

#### Option 3: Serverless (Vercel, Netlify, etc.)

For serverless deployment, you may need to adapt the code to work with serverless functions. The interactive chat mode requires persistent connections, so single query mode works better for serverless.
...
3. Use OpenAI to answer the question
